...
| [`rust/build`](rust/build/action.yml) | Run Rust unit tests and install `cargo-vsql` |
| [`rust/test`](rust/test/action.yml) | Download dev server and run `cargo vsql test` |
...
Your extension repo must be a Cargo workspace or crate that `cargo vsql test` can build and test. The packaged `.veb` is expected at `dist/<extension-name>.veb` relative to the extension directory.
...
jobs:
  ci:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: villagesql/extension-actions/rust/build@main
      - uses: villagesql/extension-actions/rust/test@main
        with:
          extension-name: my_extension
          github-token: ${{ secrets.GITHUB_TOKEN }}
```
...
- uses: villagesql/extension-actions/rust/build@v1
- uses: villagesql/extension-actions/rust/test@v1
```
